...
export default function ReportModal({ answerId, onClose }: ReportModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/answers/${answerId}/reports`, {
        reason,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="flex items-center justify-between px-6 py-4 border-b border-rose-100">
          <h3 className="text-lg font-semibold text-gray-800">Cevabı Şikayet Et</h3>
          <button
            onClick={onClose}
